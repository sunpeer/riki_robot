...
//int M1dirpin = 31;  //电机X方向引脚
//int M1steppin = 12;  //电机X驱动引脚
//int M1en=18;        //电机X使能引脚
...
  pinMode(Mlsteppin,OUTPUT);
  pinMode(Mlen,OUTPUT);
  digitalWrite(Mlen,LOW); //低电平使能
}
...
  for(j=0;j<=5000;j++)
  {
    digitalWrite(Mlsteppin,LOW);
    delayMicroseconds(2);
    digitalWrite(Mlsteppin,HIGH);
    delay(1);
	}
}
